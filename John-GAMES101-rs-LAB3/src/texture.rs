...
impl Texture {
    ...
    pub fn get_color_bilinear(&self, mut u: f64, mut v: f64) -> Vector3<f64> {
        ...
        let u_min = u_img.floor();
        let u_max = u_img.ceil();
        let v_min = v_img.floor();
        let v_max = v_img.ceil();

        let color00: &VecN<u8, 3> = self.img_data.at_2d(v_max as i32, u_min as i32).unwrap();
        let color10: &VecN<u8, 3> = self.img_data.at_2d(v_min as i32, u_min as i32).unwrap();
        let color01: &VecN<u8, 3> = self.img_data.at_2d(v_max as i32, u_max as i32).unwrap();
        let color11: &VecN<u8, 3> = self.img_data.at_2d(v_min as i32, u_max as i32).unwrap();
        ...
        let s: f64 = (u_img - u_min) / (u_max - u_min);
        let t: f64 = (v_img - v_max) / (v_min - v_max);
        let u0 = u00 + s *(u01 - u00);
        let u1 = u10 + s *(u11 - u10);
        let color = u0 + t *(u1 - u0);
        Vector3::new(color[2] as f64, color[1] as f64, color[0] as f64)
    }
}